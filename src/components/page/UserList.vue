    ...
    <div>
        ...
        <div class="container">
            <div class="handle-box">
                <el-select v-model.trim="query.member" placeholder="是否会员" class="handle-select mr10" @change="onSelectMember($event)">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header">
                ...
                <el-table-column label="头像" align="center">
                    <template slot-scope="scope">
                        <el-image class="table-td-thumb" :src="scope.row.userImage" :preview-src-list="[scope.row.userImage]"></el-image>
                    </template>
                </el-table-column>
                <el-table-column prop="userIntegral" label="积分" align="center"></el-table-column>
                <el-table-column label="是否会员" align="center">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.isVip === 1 ? 'success' : scope.row.isVip === 0 ? 'danger' : ''"
...
export default {
    ...
    methods: {
        ...
        getData() {
            let vueData = {
                token: '252854585',
                isMember: this.query.member,
                search: this.query.search,
                pageIndex: this.pageIndex,
                pageSize: this.pageSize
            };
            userList(vueData).then((res) => {
                if (res.code == 10000) {
